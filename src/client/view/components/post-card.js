...
import {
  ...
  ICON_RESPONSE_ACCEPT, ICON_RESPONSE_REJECT,
 ...
 * @returns {HTMLElement}
 */
export function renderPostCard(record, permissions, currentUser, handlers = {}) {
  const {
  ...
  const stateClass = isStanding ? 'post-card--standing'
                   : isUntested ? 'post-card--untested'
                   : '';

  const card = document.createElement('div');
  card.className = `card post-card post-card--${record.type} ${stateClass}`.trim();
  ...
  // Click-anywhere-to-open (FR-051) — fires on card itself, not buttons
