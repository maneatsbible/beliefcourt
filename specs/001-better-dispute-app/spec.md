...
- **FR-162** (**Bible Study Duel**): A `context=bible_study` Duel MAY be filed on any Doctrinal Claim. Both parties ground their positions in scripture. The Evidence panel prominently surfaces all cited scripture references. A trusted panel of up to 7 Judges MAY be invited. Judges may cite their own Scripture Evidence in their Analysis. Turn prompts ask *"What does this passage mean to you?"* not *"What argument does this verse support?"*
...
- **FR-169** (**Exploring Our Faith**): A Person MAY tag any ClaimAccord as `faith_relevant`. Faith-tagged ClaimAccords form a dedicated **Exploring Our Faith** profile section and compatibility dimension. In Dating Mode and Match Profile, faith alignment is shown as a separate score from general claim alignment. Exploring Our Faith surfaces potential matches who share faith positions with the viewer. Opt-in only. Accessible at `/persons/:id/faith` and as a Match Profile filter. This is a primary discovery surface for the faith community.
- **FR-170** (**Tradition tag**): A Person MAY optionally tag their profile with a `tradition` string (e.g. `"Reformed"`, `"Catholic"`, `"Pentecostal"`, `"Baptist"`, `"Anglican"`). Free-text, not a controlled vocabulary. Displayed on the Person profile card and in Exploring Our Faith. Never used algorithmically to route users, filter content, or weight verdicts. Informational only.
