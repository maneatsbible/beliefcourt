...
- **FR-164** (**Pre-marital Counselling Track**) *(secondary priority — ships after core dispute and reconciliation features)*: A couple in `engaged` MarriageRecord state MAY activate a **Pre-marital Counselling Track** — a structured series of up to 12 pre-built `context=decision` Duel templates covering: finances, children, extended family roles, faith practice, conflict resolution, intimacy, vocation, and shared goals. A counsellor or pastor invited as an External Counsellor (FR-151) may see all track Duels and add private Analysis. Track progress is shown at `/marriages/:id/premarital`.
...
- **FR-167** (**Church Discipline**): A `context=discipline` process MAY be initiated by an org elder against an org member. The process has three mandatory stages consistent with Matthew 18: (1) private `context=apology` Duel between the elder and the member; (2) if unresolved, a second `context=apology` Duel with two additional Witnesses required; (3) if unresolved, escalation to a `context=discernment` Duel before the full elder panel. Each stage MUST be completed before the next is available. Stage records are private to participants. No outcome is published to the org feed without explicit consent of all parties. The system never uses the word "discipline" in the UI — stages are labelled **"Reconciliation"**, **"Witnessed Reconciliation"**, and **"Community Review"**. The Matthew 18 structure is documented as a comment in the discipline-context controller; it is never rendered to the user.
...
- **FR-169** (**Exploring Our Faith**): **Exploring Our Faith** is a primary, ongoing, structured project — not a filter or a feed section. Its purpose is to map the real variations in Christian belief across traditions, present them as interactive **Catechisms** for learning and challenge, and draw people closer to Jesus Christ through honest engagement with what Christians actually believe and why. It is the deepest expression of Christian Mode and MUST be treated as a core feature with its own product surface.

  **What it is:**
  - A dedicated section at `/faith` (accessible from main navigation for all Christian Mode users) containing: (1) a **Tradition Map**, (2) a **Catechism Library**, and (3) a **Personal Faith Profile** tied to a Person's ClaimAccords and Duel history.
  - It is long-lived and grows over time. New Catechisms, new tradition variants, and new doctrinal question sets are added continuously as community-contributed content.
  ...
  **Personal Faith Profile:**
  ...
  - In Dating Mode and Match Profile, faith alignment is shown as a separate **Faith Alignment Score** alongside the standard compatibility score. This score reflects how many faith-tagged ClaimAccords the two Persons share (agree on) vs. dispute. The score is not a ranking — it is an informational signal.
  - Exploring Our Faith surfaces potential matches who share faith positions with the viewer. Used as a Match Profile filter in the Christian Dating feed.

  **Tradition Map:**
  - A visual, interactive map of Christian traditions (Reformed, Catholic, Eastern Orthodox, Pentecostal, Baptist, Anglican, Lutheran, Methodist, Non-denominational, Anabaptist, etc.) organised by shared doctrinal commitments rather than denominational labels.
  - Each tradition cluster shows: the key doctrines it holds, how those doctrines are contested within and across traditions, and which Catechism questions are most divisive or most agreed-upon within it.
  - Users on the Tradition Map can see how their own Personal Faith Profile aligns with each tradition cluster — not to push them toward a label, but to help them understand where they stand and why.
  - The Tradition tag (FR-170) is the user-supplied anchor; the Map is the system's structural view. A user with no tradition tag still has a position on the Map based on their ClaimAccords.

  **Catechism Library:**
  - A library of structured **Catechisms** — sets of doctrinal questions, each with a standard position, a standard counterposition, and supporting scripture for both. Every Catechism question is a Doctrinal Claim template.
  - Catechism categories at launch: Salvation (soteriology), Scripture (bibliology), Creation (cosmology), Trinity (theology proper), Christ (Christology), the Church (ecclesiology), Sacraments/Ordinances, Eschatology, Ethics and the Law, Prayer and the Spirit, Family and Covenant, and Faith Practices.
  - Each Catechism question can be engaged with in three ways: (1) **Study** — read the question, the standard positions, and the scripture references in the Bible Reader without filing a Duel; (2) **Declare** — file a ClaimAccord declaring your position without challenge; (3) **Duel** — file a `context=bible_study` or `context=doctrinal` Duel on the question and defend your position against a challenger.
  - Completing a Catechism category (engaging with all questions in it) earns a **Catechism Badge** on the Personal Faith Profile. This is not gamification for its own sake — it is a record that a Person has engaged seriously with a body of Christian thought.
  - Catechisms are community-contributed. Any Person may propose a new Catechism question; proposals are reviewed by the org elder panel or platform moderators before publication. Both positions MUST be presented neutrally and grounded in scripture — no straw-manning.
  - The Bible Widget (FR-210) is deeply integrated into the Catechism Library. Every Catechism question surfaces its scripture references as expandable Bible Widgets. Selecting "Study" on any question opens the full Bible Reader (FR-211) pre-loaded with the relevant passage.

- **FR-170** (**Tradition tag**): A Person MAY optionally tag their profile with a `tradition` string. Suggested values (not enforced): `"Reformed"`, `"Catholic"`, `"Eastern Orthodox"`, `"Pentecostal"`, `"Charismatic"`, `"Baptist"`, `"Anglican"`, `"Lutheran"`, `"Methodist"`, `"Non-denominational"`, `"Anabaptist"`, `"Evangelical"`. Free-text, not a controlled vocabulary — a user may enter any value. Displayed on the Person profile card and in Exploring Our Faith. Shown on the Tradition Map as the user's stated anchor. Never used algorithmically to route users, filter content, or weight verdicts. Informational and self-reported only.
- **FR-171** (**Christian Dating context**): A `context=christian_dating` Duel MAY be filed between two Persons who have both opted into Christian Mode. This is a structured courtship Duel: the filing Person states an intention or compatibility question grounded in shared faith (e.g. "We share a commitment to raising children in the faith"). Both parties respond with their position and supporting Evidence, including Scripture Evidence. The verdict is private by default. Key use cases: (1) faith compatibility assessment before committing to a relationship, (2) doctrinal alignment check between dating Christians, (3) shared values declaration early in courtship. AI turn prompts in this context ask *"What does your faith say about this?"* and *"How would you pursue this together?"* — never framing it as a debate to be won. The Score Card for Christian Dating Duels shows a **Faith Alignment Score** alongside the standard compatibility score.
...
- **FR-210** (**Bible Widget**): A **Bible Widget** MAY be attached to any Record (Claim, Turn, Analysis, Judgment, Covenant line). A Bible Widget consists of: `ref` (human-readable reference, e.g. `"Romans 8:28–30"`), `verseIds` (canonical api.bible verse ID array), and `translation` (default: `"KJV"`). The widget renders as a collapsed pill showing the reference and translation badge (e.g. `Romans 8:28–30 · KJV ▾`). Expanding the pill shows the verse text, individually numbered, with line-height optimized for reading. A **"Open in Bible Reader"** button opens the full Bible Reader (FR-211) pre-loaded to the passage. The Bible Widget is the first implemented Widget type. Composer integration allows any user to attach a Bible passage by book/chapter/verse selection or by reference search. Multiple Bible Widgets MAY be attached to a single Record.
  ...
  **Future translation integrations** (roadmap only — not first release): ESV API (free non-commercial, requires attribution), NET Bible (free with attribution), BSB (Berean Standard Bible, free with attribution), NASB (licensed), NIV (licensed), NKJV (licensed). Licensed translations MAY be added as user-provided API keys in Settings — the platform provides the UI; the user provides the key. This is the "bring your own license" model: no platform cost, no copyright risk, user retains control.
